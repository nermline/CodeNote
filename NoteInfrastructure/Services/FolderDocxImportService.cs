...
namespace NoteInfrastructure.Services
{
    public class FolderDocxImportService : IImportService<Folder>
    {
        ...
        {
            if (!stream.CanRead) throw new ArgumentException("Потік не може бути прочитаний.", nameof(stream));
            ...
            var body = doc.MainDocumentPart?.Document?.Body ?? throw new InvalidOperationException("Документ не містить тіла.");
        ...
        private async Task HandleParagraphAsync(Paragraph para, ImportState state, Dictionary<string, Folder> folderCache, Dictionary<string, File> fileCache, Dictionary<string, NoteTag> tagCache, CancellationToken ct)
        {
            ...
            if (state.CurrentFolder is not null && text.StartsWith(FolderDatePrefix, StringComparison.OrdinalIgnoreCase))
            {
        ...
        private async Task HandleTableAsync(Table table, ImportState state, Dictionary<string, NoteTag> tagCache, CancellationToken ct)
        {
            ...
            if (state.InFileMeta)
            {
                foreach (var row in rows)
                {
                    ...
                    if (label.Equals(LabelDescription, StringComparison.OrdinalIgnoreCase)) state.CurrentFile.Description = value;
                    if (label.Equals(LabelTags, StringComparison.OrdinalIgnoreCase)) await AttachTagsAsync(value, state.CurrentFile, tagCache, ct);
                    if (label.Equals(LabelFileDate, StringComparison.OrdinalIgnoreCase) && TryParseDate(value, out var fileDate)) state.CurrentFile.Createdat = fileDate;
                }
                return;
            }
            ...
            if (state.InVersionBlock && state.CurrentVersion is not null)
            {
                foreach (var row in rows)
                {
                    ...
                    if (label.Equals(LabelChangelog, StringComparison.OrdinalIgnoreCase)) state.CurrentVersion.Changelog = value;
                    if (label.Equals(LabelVersionDate, StringComparison.OrdinalIgnoreCase) && TryParseDate(value, out var versionDate)) state.CurrentVersion.Createdat = versionDate;
            ...
            foreach (var row in rows)
            {
                ...
                if (state.CurrentVersion is not null)
                {
                    if (label.Equals(LabelChangelog, StringComparison.OrdinalIgnoreCase)) state.CurrentVersion.Changelog = value;
                    if (label.Equals(LabelVersionDate, StringComparison.OrdinalIgnoreCase) && TryParseDate(value, out var versionDate)) state.CurrentVersion.Createdat = versionDate;
                    if (label.Equals(LabelContent, StringComparison.OrdinalIgnoreCase)) state.CurrentVersion.Content = value;
                }
            }
        }
        ...
        private async Task AttachTagsAsync(string rawTags, File file, Dictionary<string, NoteTag> cache, CancellationToken ct)
        {
            ...
            foreach (var tagName in tagNames)
            {
                ...
                if (!cache.TryGetValue(safeName, out var tag))
                {
                    ...
                    if (tag is null) { tag = new NoteTag { Name = safeName }; _context.Tags.Add(tag); }
                    cache[safeName] = tag;
                }
                file.Tags.Add(tag);
            }
        ...
        private static int NextVersionNumber(File file) => file.Fileversions.Any() ? file.Fileversions.Max(v => v.Versionnumber) + 1 : 1;
